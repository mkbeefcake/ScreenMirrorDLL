...
BOOL StartSearchWindowDialog(UINT monitors, INT& selectedMonitor, HWND& selectedWindow)
{
	...
	g_hwndMainWnd = GetDesktopWindow();
	...
	selectedWindow = g_hwndFoundWindow;
...
)
{
	...
	wsprintf(wszText, L"%d", screenpoint.x);
	SetDlgItemText(hwndDialog, IDC_STATIC_X_POS, wszText);

	wsprintf(wszText, L"%d", screenpoint.y);
	SetDlgItemText(hwndDialog, IDC_STATIC_Y_POS, wszText);
...
long DisplayInfoOnFoundWindow(HWND hwndDialog, HWND hwndFoundWindow)
{
	...
	GetClassName(hwndFoundWindow, wszClassName, sizeof(wszClassName) - 1);

	// Display some information on the found window.
	wsprintf
	(
		wszText, L"Window Handle == 0x%08X.\r\nClass Name : %s.\r\nRECT.left == %d.\r\nRECT.top == %d.\r\nRECT.right == %d.\r\nRECT.bottom == %d.\r\n",
		hwndFoundWindow,
		wszClassName,
		rect.left,
		rect.top,
	...
	SetDlgItemText(hwndDialog, IDC_EDIT_STATUS, wszText);
	globalSearchedWindow = hwndFoundWindow;
...
)
{
	...
	case WM_COMMAND:
	{
		...
		//if (wID == IDC_ACTION)
		//{
		//	if (globalRegisteredLast < 5) {
		//		globalRegisteredWindow[globalRegisteredLast] = globalSearchedWindow;
		//		::ShowWindow(globalRegisteredWindow[globalRegisteredLast], SW_HIDE);
		//		globalRegisteredLast++;
		//	}
		//}
